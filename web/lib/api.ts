...
export const api = {
  ...
  dashEngagement: (days = 30) => req("GET", `/admin/dash/engagement?days=${days}`),
  dashBackfill: (days = 30) => req("POST", `/admin/dash/backfill?days=${days}`),
  dashRevenue: (days = 30) => req("GET", `/admin/dash/revenue?days=${days}`),
  dashPoker: (days = 30) => req("GET", `/admin/dash/poker?days=${days}`),
  dashBots: () => req("GET", "/admin/dash/bots"),
  dashLeague: () => req("GET", "/admin/dash/league"),
  dashBehaviour: () => req("GET", "/admin/dash/behaviour"),
  ...
  clubEdit: (b: unknown) => req("PATCH", "/clubs", b),
  ...
  myClub: () => req("GET", "/clubs/me"),
  createClub: (b: unknown) => req("POST", "/clubs", b),
  joinClub: (code: string) => req("POST", "/clubs/join", { code }),
  leaveClub: () => req("POST", "/clubs/leave"),
  clubBrowse: (q: string) => req("GET", "/clubs/browse?q=" + encodeURIComponent(q)),
  clubLeaderboard: () => req("GET", "/clubs/leaderboard"),
  clubPromote: (id: number) => req("POST", "/clubs/members/promote", { user_id: id }),
  clubDemote: (id: number) => req("POST", "/clubs/members/demote", { user_id: id }),
  clubKick: (id: number) => req("POST", "/clubs/members/kick", { user_id: id }),
  clubMessages: (after: number) => req("GET", "/clubs/messages?after=" + after),
  clubSend: (text: string) => req("POST", "/clubs/messages", { text }),
  clubRequests: () => req("GET", "/clubs/requests"),
  clubApprove: (id: number) => req("POST", "/clubs/requests/approve", { user_id: id }),
  clubReject: (id: number) => req("POST", "/clubs/requests/reject", { user_id: id }),
  clubGames: () => req("GET", "/clubs/games"),
};
